...
EOF
```
> etant donne que je suis sur VM, et que ma machine hote est en UTC+2, je configure l'horloge materielle en heure locale (UTC=0). A vous d'adapter si besoin
...
EOF
```
> adapter la police si besoin, possible d'ajouter `KEYMAP` si vous voulez definir un layout clavier specifique. + d'exemples et d'infos [ici](https://www.linuxfromscratch.org/~xry111/lfs/view/arm64/chapter09/usage.html)

### Configurer System Locale
...
EOF
```
> J'ai préféré l'anglais US, mais on peut choisir n'importe quelle autre locale disponible. Si tu as suivi le tuto jusqu'ici, tu devrais toutes les avoirs

### Configuration `/etc/inputrc`
...
EOF
```
> Sert a configurer le comportement de l'input dans les shells interactifs, fichier global pouvant ensuite etre surchargé par un user a la racine de son home `~/.inputrc`
> Pas mal pratique pour faire sauter le son du terminal avec `set bell-style none`
...
EOF
```
> Liste des shells valides pour le systeme, utile pour certains programmes comme `chsh` ou `login`
